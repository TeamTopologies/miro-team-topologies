...
function getImage(img: {src: string}) {
...
function addShapes(container: HTMLElement) {
...
function addImages(container: HTMLElement) {
  container.innerHTML += images.map((i) => getImage(i)).join('')
}

function createImage(canvasX: number, canvasY: number, url: string | null) {
...
function createShape(canvasX: number, canvasY: number, color: string | null, text: string | null) {
  return miro.board.widgets.create({
    type: 'shape',
    text: text,
    x: canvasX,
...
function bootstrap() {
  const container = document.getElementById('tt-content-panel')
  if (!container) {
    return
  }
  ...
  const imageOptions: SDK.DraggableItemsContainerOptions = {
  ...
  let currentShapeColor: string | null
  let currentShapeText: string | null
  const shapeOptions = {
